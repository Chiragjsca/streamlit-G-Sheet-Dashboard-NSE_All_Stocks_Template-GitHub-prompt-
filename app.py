...
def get_gspread_client():
    ...
    Priority:
      1. st.secrets["gcp_service_account"]  → used on Streamlit Cloud
      2. Local JSON key file                → used for local development
    """
    creds_dict = None
    source = ""

    # --- Method 1: Streamlit secrets (Streamlit Cloud deployment) ---
    try:
    ...
    # --- Method 2: Local JSON key file (local development fallback) ---
    if creds_dict is None:
        for fname in [
            "axiomatic-idiom-496012-p8-f42364b00442.json",
    ...
    if creds_dict is None:
        return None, (
            ...
            "**On Streamlit Cloud:** Go to App Settings → Secrets and paste your secrets.toml content.\n\n"
            "**Locally:** Place your JSON key file in the same folder as app.py."
        )

    # Fix escaped newlines in private_key (safe to run in both cases)
    ...
    try:
        ...
        err_msg = str(e)
        if "invalid_grant" in err_msg or "Invalid JWT" in err_msg:
            return None, (
                "❌ **Authentication failed: Invalid JWT Signature.**\n\n"
                "This means the private key in your secrets does NOT match the key registered in Google Cloud.\n\n"
                "**Fix:** Make sure you are using the credentials from your latest JSON key file "
                f"(`private_key_id` should be `f42364b0044247d5f75785deb1a68d8c51cf87a3`).\n\n"
                f"Technical error: `{err_msg}`"
            )
        return None, f"❌ Google Auth error: {err_msg}"


@st.cache_data(ttl=300, show_spinner=False)
def load_sheet(tab_name: str) -> tuple:
    """Load a single worksheet into a DataFrame. Returns (df, error_or_None)."""
    ...
    try:
        ...
        return pd.DataFrame(), f"Sheet '{tab_name}' not found. It may not exist in your Google Sheet yet."
...
def show_searchable_table(df: pd.DataFrame, key: str):
    """Display DataFrame with a search/filter box."""
...
if group == "🏠 Overview":
    ...
    with col5:
        st.markdown(f'<div class="metric-card"><div class="metric-val">{above200}</div><div class="metric-lbl">Above 200 DMA</div></div>', unsafe_allow_html=True)
    ...
    with col_l:
        ...
        if "% Change" in df_num.columns:
            fig = px.histogram(
                df_num.dropna(subset=["% Change"]), x="% Change", nbins=40,
    ...
    show_searchable_table(df_price, "overview_price")
                ...
                if tab_name in ("Final List", "Final List-2"):
                    ...
                    with col_a:
                        if "Market Cap (Cr)" in df_num.columns and "Sector" in df.columns:
                            ...
                            mc_grp = (mc.groupby("Sector")["Market Cap (Cr)"]
                                      .sum().reset_index()
                                      .sort_values("Market Cap (Cr)", ascending=False).head(10))
